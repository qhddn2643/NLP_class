...
def preprocess_function(
    ...
    # 1. The loop below serves to label tokenized words for training the model.
    # 2. This is because the model will throw an error if there are sentences where the maximum number of tokens is greater than max_seq_length.
    # END OF YOUR ANSWER
    decoder_input_ids = []
    labels = []
    ...
    *,
    ...
    for batch in tqdm(dataloader, desc="Evaluation"):
        with torch.inference_mode():
            ...
            # YOUR ANSWER HERE (please limit your answer to 1-2 sentences):
            # 1. model.forward() always gets the correct truth token in the next step, regardless of the prediction. model.generate() uses the generated token as an input in the next step. 
            # 2. The .forward() call must have decoder_input_ids.
...
def main():
    ...
    # (readability matters)
    # YOUR CODE STARTS HERE
    train_dataloader = DataLoader(train_dataset, shuffle=True, collate_fn=collation_function_for_seq2seq_wrapped, batch_size=args.batch_size)
    eval_dataloader = DataLoader(eval_dataset, collate_fn=collation_function_for_seq2seq_wrapped, batch_size=args.batch_size)
    # YOUR CODE ENDS HERE
