...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, decoder_hidden_states, encoder_hidden_states, key_padding_mask=None):
        ...
        x = self.cross_attention(x, key_padding_mask=key_padding_mask)
...
class TransfomerEncoderDecoderModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.encoder_layers = nn.ModuleList([
            TransformerEncoderLayer(hidden, num_heads, fcn_hidden, dropout, causal=True) for _ in range(num_layers)
        ])
        self.decoder_layers = nn.ModuleList([
            TransformerEncoderLayer(hidden, num_heads, fcn_hidden, dropout, causal=False) for _ in range(num_layers)
        ])
        # YOUR CODE ENDS HERE
    ...
    def _encode(self, input_ids, key_padding_mask):
        ...
        encoder_embeds = self.encoder_embeddings(input_ids)
        positional_emb = self._add_positions(encoder_embeds)
        for encorder_layer in self.encoder_layers: 
            encoder_hidden_states = encorder_layer(encoder_embeds)
        # YOUR CODE ENDS HERE
    ...
    def _decode(self, encoder_hidden_states, decoder_input_ids, key_padding_mask):
        ...
        # YOUR CODE STARTS HERE
        decoder_embeds = self.decoder_embeddings(decoder_input_ids)
        positional_emb = self._add_positions(decoder_embeds)
        for decoder_layer in self.decoder_layers: 
            output = decoder_layer(decoder_embeds, encoder_hidden_states)
