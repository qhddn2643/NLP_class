...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, kv=None, key_padding_mask=None, return_attention=False):
        ...
        if key_padding_mask is not None:
            ...
            key_padding_mask = torch.triu(torch.ones(bs, attended_seq, device=scores.device), diagonal=4)
            scores = scores.transpose(0,1).reshape(attending_seq * self.num_heads, bs, attended_seq)            
            scores = scores.masked_fill_(key_padding_mask==1, float("-inf")).reshape(bs * self.num_heads, attending_seq, attended_seq).contiguous()
            # YOUR CODE ENDS HERE
