<!-- #region -->
# Final Project. MT5 machine translation model

In final project, I finished MT5 implementation and train a machine translation model.

I have tested this code with a cleaned version of the CodeXGLUE text-to-text dataset (`code_x_glue_tt_text_to_text`).
There are four language pairs: Danish-English(da_en), Latvian-English(lv_en), Norwegian-English(no_en), chinese-English(zh_en)
It is a large-size dataset that contains 164K sentence pairs which are enough to train a decent translation system.

> If you want to use a different language pair, just change the --dataset_config_name argument. For example, if you want to use Danish-English(da_en), set the argument of --dataset_config_name da_en


## 1. Train tokenizers for dataset

Go to `cli/create_tokenizer` and train a tokenizer. 
...
python cli/create_tokenizer.py \
    --dataset_name code_x_glue_tt_text_to_text \
    --dataset_config da_en \
    --vocab_size 32_000 \
    --save_dir da_en_output_dir \
    --source_lang da \
    --target_lang en
...
<!-- #region -->
## 2. Train a Transformer Machine Translation Model
...
In the beginning, try out a very small model in `--debug` mode to make sure the script can finish without failing. Then, find the largest batch size your GPU memory can fit and the largest learning rate that does not cause model to diverge. Also feel free to play with any other hyperparameters, such as model size learning rate schedule, regularization, and so on.
...
python cli/train.py \
	...
	--num_train_epochs 1 \
	--learning_rate 5e-5 \
	--dataset code_x_glue_tt_text_to_text \
	--device cuda \
	--source_lang da \
	--target_lang en \
	--dataset_config_name da_en \
	--output_dir da_en_output_dir \
```

## Try out your model

After training, use `notebooks/FinalDemo.ipynb` to translate texts using your model. This part is evaluated and might be rewarding, as the final system should produce reasonable translations, please do not skip it.
